...
#import "IOKeycodeUsage.h"

@implementation IOKeycodeUsage

NSDictionary* _keycodesUsageNames;

+(NSArray*)allUsageNames
{
    return self.keycodesUsageNames.allValues;
}

+(NSDictionary*)keycodesUsageNames
{
    if (!_keycodesUsageNames)
    {
        _keycodesUsageNames = @{
                              // 00 Reserved
    ...
    return _keycodesUsageNames;
}
+(NSString*)nameOfKeycodeUsage:(uint32_t)key
{
    NSString* name = self.keycodesUsageNames[@(key)];
